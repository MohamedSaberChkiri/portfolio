...
<span className="text-[13px]">TECHNOLOGIES</span>
        <span
          className={`transition-transform ${
            isClicked ? "rotate-180 duration-500 arrowslide" : "rotate-0 duration-500 arrowslideback"
          }`}
        >
          <IoIosArrowDown />
        </span>
            ...
            <div className={` flex flex-wrap gap-2 mx-auto border-white ${isClicked ? 'flex showTechs' : 'hidden hideTechs'}`}>
                {props.techs.map((tech, index) => (
                    <Technologies key={index} technologie={tech.name} icon={tech.icon} />
                ))}
            </div>
